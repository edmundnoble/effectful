...
object EffectfulSpec extends Properties("effectful") {
  ...
  property("nested traverse with multiple effects") = {
    ...
    def test[M[_], A, B](implicit
      ...
      val value = effectfully {
        for (ma <- mtma.!; mb <- mtmb.!) yield (ma !, mb !)
      }
  ...
  property("looping with state monad") = {
    forAll { (n: Int) =>
      val value = effectfullyU[IntState, Unit] {
  ...
  implicit def eitherArb[A, B](implicit A: Arbitrary[A], B: Arbitrary[B]): Arbitrary[A \/ B] =
    Arbitrary(Gen.oneOf(A.arbitrary.map(_.left[B]), B.arbitrary.map(_.right[A])))

  def divBySubOrFail(x: Int): String \/ Int = x match {
    case 0 ⇒ "failed".left
    case n ⇒ (n - 1).right
  }

  property("fail-fast with disjunction monad") = forAll { (d: String \/ Int) ⇒
    type StringOr[B] = String \/ B
    effectfullyU[StringOr, Int] {
      divBySubOrFail(divBySubOrFail(d.!).!).!
    } == (d flatMap divBySubOrFail flatMap divBySubOrFail)
  }

  // TODO: implement function values, or switch to Scalaprops. As it is, I don't believe scalacheck does a good job
  ...
  property("traversal with pure filter") = {
    ...
    def test[M[_], A, B](implicit
      ...
      val value = effectfully {
        for (xs <- xss; if ptma(xs); x <- xs) yield f(x !)
      }
  ...
  property("traversal with impure filters") = {
    ...
    def test[M[_], A, B](implicit
      ...
      val value = effectfully {
        for (xs <- xss; if ptma(xs !); x <- xs !; if pa(x !)) yield f(x !)
      }
  ...
  property("custom traverse type") = {
    ...
    val value = effectfully {
      xs withFilter (_ > 0) map (_ * Option(4).!)
    }
    val expected = xs.traverse[Option, Int](x => Option(4) map (y => x * y))
    value == expected
  }
}